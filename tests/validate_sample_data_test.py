...
def test_track_id_type(data):
    ...
    for val in data["track_id"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.track_id.can_be_none)
...
def test_streams_type(data):
    ...
    for val in data["streams"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.streams.can_be_none)
...
def test_artist_followers_type(data):
    ...
    for val in data["artist_followers"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.artist_followers.can_be_none)
...
def test_genres_type(data):
    ...
    for val in data["genres"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.genres.can_be_none)
...
def test_album_total_tracks_type(data):
    ...
    for val in data["album_total_tracks"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.album_total_tracks.can_be_none)
...
def test_track_artists_type(data):
    ...
    for val in data["track_artists"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.track_artists.can_be_none)
...
def test_artist_popularity_type(data):
    ...
    for val in data["artist_popularity"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.artist_popularity.can_be_none)
...
def test_explicit_type(data):
    ...
    for val in data["explicit"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.explicit.can_be_none)
...
def test_tempo_type(data):
    ...
    for val in data["tempo"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.tempo.can_be_none)
...
def test_chart_type(data):
    ...
    for val in data["chart"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.chart.can_be_none)
...
def test_album_release_date_type(data):
    ...
    for val in data["album_release_date"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.album_release_date.can_be_none)
...
def test_energy_type(data):
    ...
    for val in data["energy"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.energy.can_be_none)
...
def test_key_type(data):
    ...
    for val in data["key"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.key.can_be_none)
...
def test_added_at_type(data):
    ...
    for val in data["added_at"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.added_at.can_be_none)
...
def test_popularity_type(data):
    ...
    for val in data["popularity"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.popularity.can_be_none)
...
def test_track_album_album_type(data):
    ...
    for val in data["track_album_album"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.track_album_album.can_be_none)
...
def test_duration_ms_type(data):
    ...
    for val in data["duration_ms"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.duration_ms.can_be_none)
...
def test_available_markets_type(data):
    ...
    for val in data["available_markets"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.available_markets.can_be_none)
...
def test_track_track_number_type(data):
    ...
    for val in data["track_track_number"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.track_track_number.can_be_none)
...
def test_rank_type(data):
    ...
    for val in data["rank"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.rank.can_be_none)
...
def test_mode_type(data):
    ...
    for val in data["mode"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.mode.can_be_none)
...
def test_time_signature_type(data):
    ...
    for val in data["time_signature"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.time_signature.can_be_none)
...
def test_album_name_type(data):
    ...
    for val in data["album_name"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.album_name.can_be_none)
...
def test_speechiness_type(data):
    ...
    for val in data["speechiness"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.speechiness.can_be_none)
...
def test_region_type(data):
    ...
    for val in data["region"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.region.can_be_none)
...
def test_danceability_type(data):
    ...
    for val in data["danceability"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.danceability.can_be_none)
...
def test_valence_type(data):
    ...
    for val in data["valence"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.valence.can_be_none)
...
def test_acousticness_type(data):
    ...
    for val in data["acousticness"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.acousticness.can_be_none)
...
def test_liveness_type(data):
    ...
    for val in data["liveness"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.liveness.can_be_none)
...
def test_trend_type(data):
    ...
    for val in data["trend"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.trend.can_be_none)
...
def test_instrumentalness_type(data):
    ...
    for val in data["instrumentalness"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.instrumentalness.can_be_none)
...
def test_loudness_type(data):
    ...
    for val in data["loudness"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.loudness.can_be_none)
...
def test_name_type(data):
    ...
    for val in data["name"]:
        assert isinstance(val, data_type) \
            or (np.isnan(val) and expectations.columns_expectation.name.can_be_none)
