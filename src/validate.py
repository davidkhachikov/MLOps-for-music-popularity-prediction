...
def run_giskard_scan(giskard_model, giskard_dataset, model_name, dataset_name, version, BASE_PATH):
    scan_results = giskard.scan(giskard_model, giskard_dataset)
    scan_results_path = f"{BASE_PATH}/reports/validation_results_{model_name}_{dataset_name}_{version}.html"
...
def create_and_run_test_suite(giskard_model, giskard_dataset, model_name, dataset_name, version, threshold):
    ...
    def rmse_test(giskard_dataset):
        y_true = giskard_dataset.df[giskard_dataset.target]
        y_pred = giskard_model.predict(giskard_dataset).raw
        rmse = calculate_rmse(y_true, y_pred)
        return giskard.TestResult(description="RMSE Test", passed=rmse <= threshold, metrics={"RMSE": rmse})

    # Initialize the test suite
    test_suite = giskard.Suite(name="My Test Suite")

    # Assuming giskard_model is defined or obtained somehow above this line
    test_suite.add_test(rmse_test, test_id="RMSE_Test_ID")
...
def select_best_model(cfg, giskard_dataset, version, BASE_PATH):
    ...
    for model_name, model_alias in zip(model_names, model_aliases):
        ...
        run_giskard_scan(giskard_model, giskard_dataset, model_name, giskard_dataset.name, version, BASE_PATH)
...
def tag_and_deploy_best_model(best_model):
    if best_model:
        ...
        print(f"Model {model_name} alias {model_alias} is tagged as the champion and deployed.")
    else:
...
def main():
    ...
    giskard_model = load_and_wrap_mlflow_model("models:/neural_network@challenger1", feature_names, cfg)
    y_pred = giskard_model.predict(giskard_dataset).raw
