...
main() {
  ...
  chmod 777 ./data/samples/
    for i in {1..5}; do
        ...
        git commit -m "Save validated data number ${i}" || { echo "Failed to commit changes"; read -p "Press Enter to exit..."; exit 1; }
        git tag -a "1.${i}" -m "Version tag" || { echo "Failed to create tag"; read -p "Press Enter to exit..."; exit 1; }
